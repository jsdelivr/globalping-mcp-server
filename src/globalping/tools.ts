...
function calculateAverages(results: any) {
  ...
  for (const item of successfulResults) {
    ...
    if (item.result.stats.min !== null) {
      summary.minRtt = summary.minRtt === null ? item.result.stats.min 
        : Math.min(summary.minRtt, item.result.stats.min);
    }
    
    if (item.result.stats.max !== null) {
      summary.maxRtt = summary.maxRtt === null ? item.result.stats.max 
        : Math.max(summary.maxRtt, item.result.stats.max);
    }
  }

  // Calculate averages
  summary.averageLoss = summary.totalPackets > 0 
    ? (summary.totalDropped / summary.totalPackets) * 100 
    : 0;
      ...
      measurement.results.forEach((result: any, index: number) => {
        ...
        if (testResult.status === 'finished' && testResult.hops) {
          testResult.hops.forEach((hop: any, hopIndex: number) => {
            ...
            const rtt = hop.timings && hop.timings[0] ? `${hop.timings[0].rtt.toFixed(2)} ms` : 'N/A';
...
export function registerGlobalpingTools(server: McpServer) {
  ...
  const baseParams = {
    ...
    locations: z.array(z.string()).optional().describe('Specific locations to run the test from using the magic field syntax'),
    limit: z.number().optional().describe('Number of probes to use (default: 3)')
  };
  ...
  server.tool(
    ...
    {
      ...baseParams,
      protocol: z.enum(['ICMP', 'TCP', 'UDP']).optional().describe('Protocol to use (default: ICMP)'),
  ...
  server.tool(
    ...
    {
      ...baseParams,
      queryType: z.enum(['A', 'AAAA', 'MX', 'NS', 'TXT', 'CNAME', 'SOA', 'CAA']).optional().describe('DNS record type (default: A)'),
  ...
  server.tool(
    'mtr',
    {
      ...
      protocol: z.enum(['ICMP', 'TCP', 'UDP']).optional().describe('Protocol to use (default: ICMP)'),
      port: z.number().optional().describe('Port number for TCP/UDP (default: 80)'),
      packets: z.number().optional().describe('Number of packets to send to each hop (default: 3)')
    },
  ...
  server.tool(
    'http',
    {
      ...
      protocol: z.enum(['HTTP', 'HTTPS']).optional().describe('Protocol to use (default: auto-detect from URL)'),
  ...
  server.tool(
    'globalping',
    {
      query: z.string().describe('The network query to run (e.g., "ping google.com", "traceroute 1.1.1.1")'),
      locations: z.array(z.string()).optional().describe('Specific locations to run the test from')
    },
    async ({ query, locations }, ctx) => {
      const token = extractApiToken(ctx);
...
/**
 * Extract the Globalping API token from request headers or context
 * @param ctx The MCP server context
 * @returns The API token if available
 */
function extractApiToken(ctx: any): string | undefined {
  // The user's bearer token is passed via props from the auth header
  if (ctx?.props?.bearerToken) {
    // Return the full bearer token string for use with the API
    return ctx.props.bearerToken;
  }
