...
app.get("/authorize", async (c) => {
  ...
  if (`${new URL(c.req.url).origin}/auth/callback` !== oauthReqInfo.redirectUri && !/http:\/\/localhost:\d+\/(.*)/is.test(oauthReqInfo.redirectUri) && !isDeepLink(oauthReqInfo.redirectUri) && !isExceptionHost(oauthReqInfo.redirectUri)) {
